...
if (isFirebaseConfigured && auth) {
  (function initAuth() {
    ...
    async function signInWithGoogle() {
      ...
      try {
        ...
        try {
          ...
          if (code === 'auth/popup-blocked' || code === 'auth/cancelled-popup-request' || code === 'auth/internal-error' || popupErr) {
    ...
    auth.onAuthStateChanged((user) => {
      if (user) {
        ...
        if (!pending) {
          ...
          // If a Google Auth redirect is in progress, wait for getRedirectResult to finish
          setTimeout(() => {
    ...
    async function showApp(user) {
      ...
      // Update Topbar Profile Avatar & Dropdown & Mobile Profile Tab Panel
      ...
      const mobileProfileImg = document.getElementById('mobile-profile-user-img');
      const mobileProfileInitial = document.getElementById('mobile-profile-user-initial');
      const mobileProfileName = document.getElementById('mobile-profile-user-name');
      const mobileProfileEmail = document.getElementById('mobile-profile-user-email');
      const mobileProfileBadge = document.getElementById('mobile-profile-user-badge');

      if (user && user.photoURL) {
        if (topbarImg) { topbarImg.src = user.photoURL; topbarImg.classList.remove('hidden'); }
        if (topbarInitial) topbarInitial.classList.add('hidden');
        if (mobileProfileImg) { mobileProfileImg.src = user.photoURL; mobileProfileImg.classList.remove('hidden'); }
        if (mobileProfileInitial) mobileProfileInitial.classList.add('hidden');
      } else {
        ...
        if (mobileProfileInitial) { mobileProfileInitial.textContent = initial; mobileProfileInitial.classList.remove('hidden'); }
      }
      ...
      if (mobileProfileEmail) mobileProfileEmail.textContent = (user && user.email) || 'Local Account';
      if (mobileProfileBadge) mobileProfileBadge.textContent = genderText;
