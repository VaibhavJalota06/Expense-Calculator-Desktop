...
const viewHeadings = {
  ...
  profile: { title: 'User Profile & Settings', subtitle: 'Account preferences & data management' }
};
...
function updateUI() {
  ...
  if (btnEditBudget) {
    btnEditBudget.innerHTML = budget > 0 
      ? '<i class="fa-solid fa-pen-to-square"></i> <span class="btn-text">Edit Budget</span>' 
      : '<i class="fa-solid fa-sliders"></i> <span class="btn-text">Set Budget</span>';
  }
...
// Profile View Panel Action Delegation
document.addEventListener('click', (e) => {
  if (e.target.closest('#btn-profile-edit')) {
    e.preventDefault();
    const btnEdit = document.getElementById('btn-dropdown-edit-profile');
    if (btnEdit) btnEdit.click();
    else {
      const modal = document.getElementById('edit-profile-modal');
      if (modal) modal.classList.remove('hidden');
    }
  } else if (e.target.closest('#btn-profile-export-csv')) {
    e.preventDefault();
    const btnExport = document.getElementById('btn-export-csv');
    if (btnExport) btnExport.click();
  } else if (e.target.closest('#btn-profile-reset-data')) {
    e.preventDefault();
    const btnReset = document.getElementById('btn-reset-all');
    if (btnReset) btnReset.click();
  } else if (e.target.closest('#btn-profile-logout')) {
    e.preventDefault();
    const btnLogout = document.getElementById('btn-dropdown-logout');
    if (btnLogout) btnLogout.click();
    else if (typeof window.handleSignOut === 'function') window.handleSignOut(e);
  }
});
